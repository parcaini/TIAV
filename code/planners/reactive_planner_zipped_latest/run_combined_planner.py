...
parser = argparse.ArgumentParser("reactive_planner")
parser.add_argument("--base_dir", nargs='?', const="./scenarios", default="./scenarios",
                    help="Specify base directory of scenarios")
parser.add_argument("--filename", required=True, help="Specify scenario XML filename to be executed")
...
while not goal.is_reached(x_0):
    ...
    if current_count % replanning_frequency == 0:
        ...
        # plan trajectory
        optimal = planner.plan(x_0, collision_checker_scenario, cl_states=x_cl,
                               draw_traj_set=plot)  # returns the planned (i.e., optimal) trajectory
    ...
    if plot:
        visualize_planning_result(scenario=scenario, planning_problem=planning_problem, ego=ego_vehicle,
                                  ...
                                  timestep=current_count, save_path=base_dir)
